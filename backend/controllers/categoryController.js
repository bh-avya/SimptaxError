...
const createCategory = asyncHandler(async (req, res) => {
    try {
        const {name} = req.body;
        if (!name) {
            return res.json({ error: "Enter Name"});
        }
...
const deleteCategory = asyncHandler(async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({error:"Couldn't remove category"});
    }
});
...
export {createCategory, updateCategory, deleteCategory, listCategory,readCategoryByID};